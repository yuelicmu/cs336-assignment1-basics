...
def run_scaled_dot_product_attention(
    ...
    return scaled_dot_product_attn_module()
...
def run_multihead_self_attention(
    ...
    """
    multihead_self_attn_module = MultiHeadSelfAttention(d_model, num_heads)
    multihead_self_attn_module.load_state_dict({
        "w_q": rearrange(q_proj_weight, "(num_heads d_v) d_model -> num_heads d_v d_model", num_heads=num_heads),
        "w_k": rearrange(k_proj_weight, "(num_heads d_v) d_model -> num_heads d_v d_model", num_heads=num_heads),
        "w_v": rearrange(v_proj_weight, "(num_heads d_v) d_model -> num_heads d_v d_model", num_heads=num_heads),
        "w_o": o_proj_weight,
    })
    return multihead_self_attn_module(in_features)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    multihead_self_attn_module = MultiHeadSelfAttention(d_model, num_heads, theta=theta, max_seq_len=max_seq_len, 
                                                        use_rope=True, token_positions=token_positions)
    multihead_self_attn_module.load_state_dict({
        "w_q": rearrange(q_proj_weight, "(num_heads d_v) d_model -> num_heads d_v d_model", num_heads=num_heads),
        "w_k": rearrange(k_proj_weight, "(num_heads d_v) d_model -> num_heads d_v d_model", num_heads=num_heads),
        "w_v": rearrange(v_proj_weight, "(num_heads d_v) d_model -> num_heads d_v d_model", num_heads=num_heads),
...
def run_rope(
    ...
    """
    rope_module = RotaryPositionalEmbedding(theta, d_k, max_seq_len)
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    """
    softmax_module = Softmax(dim=dim)
    return softmax_module(in_features)
